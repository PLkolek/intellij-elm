...
public class TypesCompletion {
    static void types(ElmCompletionContributor c) {
        c.autocomplete(afterLeaf(e(TYPE)), TypesCompletion::exposedTypes);
        c.autocomplete(afterLeaf(e(ALIAS)), TypesCompletion::exposedConstructorlessTypes);


        c.autocomplete(afterLeaf(EQUALS, PIPE).inside(e(TYPE_DECL_NODE)), TypesCompletion::exposedTypeConstructors);
    }
    ...
    private static Collection<String> exposedTypes(CompletionParameters parameters) {
        return module(parameters.getPosition())
                .typeExports()
                .flatMap(TypesCompletion::typeCompletions)
                .collect(toList());
    }
